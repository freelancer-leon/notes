...
>
> 因此，lag 值的计算是 EEVDF 调度器中的一个关键部分，大部分的 patch set 都是为了能正确得到这个值。即使没有完整的 EEVDF 算法，也可以利用一个进程的 lag 值来将其公平地放在运行队列中；lag 值较高的进程应该先运行，从而以使整个系统的 lag 值比较均匀。
>
...
> 在这 30ms 内，每个任务都本应得到 10ms（总时间的三分之一）的 CPU 时间。A 实际上获得了 30ms，因此它积累了-20ms 的滞后值；其他两个任务由于没有获得任何 CPU 时间，所以滞后了 10ms，反映了它们应该获得的 10ms CPU 时间。
>
...
* Eligible time 不是个 delta，而是 per-task 的某个时刻，它的计算跟权重（weight）有关，和请求的时间片（slice）无关。因此在权重一样的情况下，对时间的占用是公平的
  * 后面可以看到，其实它就是每个调度实体的 `vruntime`。选择任务的时候会通过 `vruntime_eligible()` 进行合格性检查，其实就是与 **应得的虚拟运行时间 V**（论文里的“系统虚拟时间”，图里的 `virtual time`）做个比较（相当于算了个 delta），所以与之前的理论并不冲突。这一点大部分文章没有点明，是不是纠结了这么久，知道真相后是不是眼泪都要掉下来了
...
pick_next_entity(struct rq *rq, struct cfs_rq *cfs_rq)
{
    ...
    if (se->sched_delayed) { //延迟出队的任务能够被选中说明它 vlag 已经衰减成正的了
        //DEQUEUE_DELAYED 语义是将延迟任务出列，把它从运行队列删除，惩罚结束
...
$$
* 从以上推导过程我们可以看到调度实体在新队列中的虚拟 lag（$vl_i$）是借助旧队列中删除时保存的虚拟 lag（$vl'_i$）计算出来的，不需要借助旧队列上的虚拟运行时间，因为新旧队列的虚拟运行时间 base 不一样
...
static void update_curr(struct cfs_rq *cfs_rq)
{
    ...
    resched = update_deadline(cfs_rq, curr); //任务得到运行了，相应地更新调度实体的虚拟截止时间
...
* 原理方面见 [前面“滞后和睡眠”](#滞后和睡眠)
  * 不合格进程睡眠的时候在 `dequeue_entity()` 标记为延迟出队，此时 $V >= v_i$，即以下表达式不成立
    ```c
    cfs_rq->avg_vruntime >= (s64)(se->vruntime - cfs_rq->min_vruntime) * cfs_rq->avg_load
    ```
  * 由于不合格而得不到运行，`se->vruntime` 一直没增加，然而 `cfs_rq->min_vruntime` 会随着其他任务的运行逐渐变大
  * 直到 `cfs_rq->min_vruntime` 增大到上面表达式成立，意味着延迟出队的任务虚拟 lag 已经变为正的了，合格了，这个时候它会被 `pick_eevdf(cfs_rq)` 选中，此时才将它正式出列（见上面的 [pick_next_entity()](#进程选择)）
    * 因此虚拟 lag 的衰减是“被动衰减”
  * 如果将来被唤醒，与正常的唤醒入列过程无异（如果是正 lag 会被带着），但如果开启了 `DELAY_ZERO` 调度器特性会导致 vlag 清零
  * 任务延迟出列期间没有被唤醒一说，因为此时它还在运行队列上，只是因不合格而得不到调度而已
```c
//kernel/sched/core.c
__schedule()
...
* 如果 vlag 从负的衰减到 `0` 或者说合格了，结束延迟出列实体的延迟状态，清除相关标志。
  * 如果开启了 `DELAY_ZERO` 调度特性，将正 vlag 清零，不要让它们获得正的 vlag
```c
...
#### 调度特性 `DELAY_DEQUEUE` 和 `DELAY_ZERO`
* 调度特性 `DELAY_DEQUEUE` 延迟退出队列的任务，直到它们被选中或唤醒。
  * 通过延迟将不符合条件的任务的出队，它们可以继续参与竞争，并消除它们的负 lag
...
```
* 调度特性 `DELAY_ZERO` 将出队（或唤醒）时的 lag 缩短为 `0`，不要让它们获得正的 vlag
  * 除了在 `finish_delayed_dequeue_entity()` 处重置 vlag 外，还有一处是任务入列的时候 `enqueue_task_fair() -> requeue_delayed_entity()`
```c
static void
requeue_delayed_entity(struct sched_entity *se)
{
    ...
    WARN_ON_ONCE(!se->sched_delayed);
    WARN_ON_ONCE(!se->on_rq);
    //如果 DELAY_ZERO 调度特性开启
    if (sched_feat(DELAY_ZERO)) {
        update_entity_lag(cfs_rq, se); //入列之前更新一下 vlag
        if (se->vlag > 0) { //如果 vlag 为正
            cfs_rq->nr_queued--;
            if (se != cfs_rq->curr) //不是当前正在运行的任务
                __dequeue_entity(cfs_rq, se); //先出列
            se->vlag = 0; //vlag 清零
            place_entity(cfs_rq, se, 0); //重新虚拟运行时间和截止时间
            if (se != cfs_rq->curr) //不是当前正在运行的任务
                __enqueue_entity(cfs_rq, se); //再入列
            cfs_rq->nr_queued++;
        }
    }
    //更新平均负载
    update_load_avg(cfs_rq, se, 0);
    clear_delayed(se); //清除延迟出列标志
}
```
