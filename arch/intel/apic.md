...
* APIC 单元内置一个 32 位可编程定时器，可供软件为各类事件或操作计时。
* 该定时器需通过配置四个寄存器完成初始化：
  * 分频配置寄存器（divide configuration register，详见图 12-10）
  * 初始计数值寄存器（initial count register）
  * 当前计数值寄存器（current count register，详见图 12-11）
  * 本地向量表定时器寄存器（LVT timer register，详见图 12-8）
* 若 `CPUID.06H:EAX.ARAT[bit 2] = 1`，则处理器的 APIC 定时器将以恒定速率运行，不受性能状态（P-state）切换的影响，且在深度休眠状态（C-state）下仍保持相同速率工作。
* 若 `CPUID.06H:EAX.ARAT[bit 2] = 0`，或处理器不支持 `CPUID 0x06` 功能，则当处理器处于深度休眠状态，或因增强型 Intel 睿频加速技术（Enhanced Intel SpeedStep® Technology） 触发状态切换时，APIC 定时器可能会暂时停止运行。
* APIC 定时器的工作频率计算方式如下：以处理器的总线时钟频率或核心晶振时钟频率为基准（当 TSC/核心晶振时钟频率之比可通过 `CPUID 0x15` 功能号枚举时，选取核心晶振时钟频率），**除以** 分频配置寄存器中设定的数值，即为 **定时器频率**。
---
* 通过配置 *本地向量表（LVT）定时器寄存器项*，可将 APIC 定时器设置为 *单触发模式* 或 *周期模式*：
#### 单触发模式
* 向 *初始计数值寄存器* 写入数值即可启动定时器，此时初始计数值会被拷贝至 *当前计数值寄存器*，随后定时器开始递减计数。
* 当计数值降至 `0` 时，定时器会产生一个定时器中断，之后便保持计数值为 `0` 的状态，直至重新对其进行配置。
#### 周期模式
* 与单触发模式相同，向 *初始计数值寄存器* 写入数值即可启动定时器，初始计数值会被拷贝至 *当前计数值寄存器* 并开始递减计数。
* 当计数值降至 `0` 时，定时器会产生中断，同时 *当前计数值寄存器* 会自动从初始计数值寄存器重新加载数值，并重复递减计数流程。
* 若在计数过程中向初始计数值寄存器写入新值，定时器将立即采用新的初始计数值重新开始计数。
---
* **初始计数值寄存器** 为 **读写** 寄存器，**当前计数值寄存器** 为 **只读** 寄存器。
![Divide Configuration Register](pic/f12-10-apic-divcfgreg.png)
![Initial Count and Current Count Registers](pic/f12-11-init-cur-cnt-reg.png)
* 在单触发模式与周期模式下，向 *初始计数值寄存器* 写入数值 `0`，均可直接停止 local APIC 定时器的运行。
* *本地向量表定时器寄存器* 决定了定时器计数值降至 `0` 时，向处理器发送的中断向量编号。
  * 该寄存器中的 *掩码标志* 可用于屏蔽定时器中断。
* **注意**：通过写入本地向量表定时器寄存器项来切换 APIC 定时器的工作模式（从单触发模式改为周期模式，或反之），并 **不会** 启动定时器。
  * 如需启动定时器，必须按照前文所述方法，向初始计数值寄存器写入数值。

#### 12.5.4.1 TSC-Deadline Mode
* local APIC 定时器的工作模式由 LVT 定时器寄存器决定，具体规则如下：
  * 若 `CPUID.01H:ECX.TSC_Deadline[bit 24] = 0`，则定时器工作模式由该寄存器的第 `17` 位决定（该寄存器的第 `18` 位为保留位）。
  * 若 `CPUID.01H:ECX.TSC_Deadline[bit 24] = 1`，则定时器工作模式由该寄存器的第 `17~18` 位决定（详见图 12-8）。
* 支持的定时器工作模式详见表 12-2。local APIC 定时器的三种工作模式为互斥关系，不可同时启用。
* Table 12-2. Local APIC Timer Modes

`LVT Bits [18:17]` | 定时器模式
-------------------|-----------------------------------------------
`00b`              | 单触发模式，在 *初始计数值寄存器* 编程递减计数值
`01b`              | 周期模式，在 *初始计数值寄存器* 编程间隔计数值
`10b`              | TSC-Deadline 模式，在 `IA32_TSC_DEADLINE` MSR 中编程目标值
`11b`              | 保留

* TSC-deadline mode 允许软件借助 local APIC 定时器，在一个绝对时间点触发中断。
  * 处于该模式时，对初始计数值寄存器的写入操作会被忽略，且当前计数值寄存器的读取值始终为 `0`。
  * 定时器的行为转而由 `IA32_TSC_DEADLINE` MSR 控制。
* `IA32_TSC_DEADLINE` MSR（地址为 `0x6E0`）是一个按逻辑处理器独立配置的寄存器，其作用是指定定时器中断的触发时间。
  * 向该寄存器写入一个非零的 64 位数值，即可启动（arm）定时器。
  * 当逻辑处理器的时间戳计数器（TSC）数值 **等于或超过** 该寄存器中设定的目标值时，硬件会产生一个定时器中断。
    * 若逻辑处理器处于 VMX non-root 模式运行状态，则对时间戳计数器的读取操作（无论是通过 `RDMSR`、`RDTSC` 还是 `RDTSCP` 指令执行），都可能无法返回时间戳计数器的实际数值。
    * VMX root 模式下运行的软件，需负责协调时间戳计数器与 `IA32_TSC_DEADLINE` MSR 的虚拟化实现。
  * 定时器触发中断后，会自动解除启动（disarm）状态，并将 `IA32_TSC_DEADLINE` MSR 的值清零。
  * 因此，对该寄存器的每一次写入操作，最多只会触发一次定时器中断。
* 在 TSC-deadline mode 下，向 `IA32_TSC_DEADLINE` MSR 写入数值 `0`，会解除 local APIC 定时器的启动状态。
* 在该模式与其他定时器模式之间进行切换时，同样会解除定时器的启动状态。
* `IA32_TSC_DEADLINE` MSR 的硬件复位值为 `0`。
  * 在其他定时器模式下（即 LVT 寄存器第 `18` 位的值为 `0` 时），该寄存器的读取值恒为 `0`，
  * 且对其执行的写入操作会被忽略。
* 软件可通过以下步骤配置 TSC-deadline 定时器，使其触发单次中断：
1. 验证 `CPUID.1:ECX.24` 的值是否为 `1`，以此检测处理器是否支持 TSC-deadline 模式。
2. 将 LVT 定时器寄存器的第 `17~18` 位配置为二进制值 `10`，从而启用 TSC-deadline 模式。
3. 向 `IA32_TSC_DEADLINE` MSR 写入期望触发定时器中断的目标 TSC 值，操作完成后处理器会启动该定时器。
4. 当时间戳计数器的数值大于或等于 `IA32_TSC_DEADLINE` 寄存器中存储的目标值时，处理器将触发定时器中断，随后自动解除定时器的启动状态，并将 `IA32_TSC_DEADLINE` 寄存器的值清零。
   * 时间戳计数器与 `IA32_TSC_DEADLINE` 寄存器均为 `64` 位无符号整数。
5. 软件如需重新启动定时器，重复执行步骤 3 即可。

##### 以下是 TSC-deadline 模式的使用准则：
* 对 `IA32_TSC_DEADLINE` MSR 的写入操作不具备串行化特性。
  * 因此，系统软件不应将向该寄存器写入数据的 `WRMSR` 指令当作串行化指令使用。对 `IA32_TSC_DEADLINE` 寄存器及其他 MSR 寄存器的读写访问，均会按照程序指令顺序执行。
  * 软件可在任意时刻向 `IA32_TSC_DEADLINE` 寄存器写入数值 `0`，以此解除定时器的启动状态。
  * 若定时器处于已启动状态，软件可通过向 `IA32_TSC_DEADLINE` 寄存器写入新值的方式，修改中断触发的截止时间点（可延后（forward）、也可提前（backward））。
  * 若软件解除定时器启动状态，或推迟了中断截止时间点，可能会因竞争条件触发与原截止时间点对应的定时器中断。
    * 若截止时间点已被推迟，软件可通过读取时间戳计数器（TSC）的值，并将其与新的截止时间点做对比，识别出这类异常触发的中断。
* 在 xAPIC mode 下（local-APIC 寄存器采用内存映射方式），软件必须对两类操作进行顺序约束：
  * 一是通过内存映射写入 LVT 寄存器项以启用 TSC-deadline 模式的操作，二是后续向 `IA32_TSC_DEADLINE` 寄存器写入数据的 `WRMSR` 指令操作。
  * 软件可通过在内存映射写入操作之后、任意 `WRMSR` 指令操作之前执行 `MFENCE` 指令，确保操作的正确顺序。
* 在 x2APIC mode 下，对 LVT 寄存器项的写入需通过 `WRMSR` 指令完成。
  * 处理器会自动保证该写入操作与后续针对 deadline 寄存器的 `WRMSR` 指令操作的执行顺序，无需额外执行内存屏障指令。
